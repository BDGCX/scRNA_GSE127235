...
ggplot(data = deg,aes(x=pct.1,y=pct.2))+#这里可能需要用abs(avg_log2FC)和-log10(p_val)画
  ...
  theme_light()+
  geom_text_repel(data=subset(deg, abs(avg_log2FC) > 2), aes(label=gene),col="black",alpha = 0.8)+
  geom_point(data=subset(deg,deg$p_val<0.05 & deg$avg_log2FC < -2),color="blue") +
  geom_point(data=subset(deg,deg$p_val<0.05 & deg$avg_log2FC > 2),color="red") 
